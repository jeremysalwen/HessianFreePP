...
template<template<typename...> class Container = std::vector, template<typename...> class SubContainer = std::vector, typename PixelType = uint8_t>
Container<SubContainer<PixelType>> read_mnist_image_file(const std::string& path,uint32_t& rows, uint32_t& columns){
    std::ifstream file;
    ...
    if(!file){
        ...
        if(magic != 0x803){
            ...
            auto frows = read_header(buffer, 2);
            auto fcolumns = read_header(buffer, 3);
			
			if(rows!=0 && frows!=rows) {
				std::cout << "Row counts do not match between data files\n";
				return {};
			}
			if(columns!=0 && fcolumns!=columns) {
				std::cout << "Column counts do not match between data files\n";
				return {};
			}
			rows=frows;
			columns=fcolumns;
...
template<template<typename...> class  Container = std::vector, template<typename...> class  SubContainer = std::vector, typename PixelType = uint8_t>
Container<SubContainer<PixelType>> read_training_images(uint32_t& rows, uint32_t& columns){
    return read_mnist_image_file<Container,SubContainer,PixelType>("mnist/train-images-idx3-ubyte",rows,columns);
}

template<template<typename...> class  Container = std::vector, template<typename...> class  SubContainer = std::vector, typename PixelType = uint8_t>
Container<SubContainer<PixelType>> read_test_images(uint32_t& rows, uint32_t& columns){
    return read_mnist_image_file<Container,SubContainer,PixelType>("mnist/t10k-images-idx3-ubyte",rows,columns);
}
...
MNIST_dataset<Container, SubContainer, PixelType> read_dataset(){
	...
	dataset.rows=0;
	dataset.columns=0;
    dataset.training_images = read_training_images<Container, SubContainer, PixelType>(dataset.rows,dataset.columns);
    dataset.training_labels = read_training_labels<Container>();

    dataset.test_images = read_test_images<Container, SubContainer, PixelType>(dataset.rows,dataset.columns);
